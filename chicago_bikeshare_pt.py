...
def most_popular_gender(data_list: list) -> str:
    """
    Função para definir qual o gênero que mais utiliza as bicicletas.
         Argumentos:
             data_list: A base de dados completa.
         Retorna:
            Uma string com o gênero de maior número.
    """
    answer = ""
...
answer = "Porque existem amostras com strings que não possuem o gênero definido ('')."
...
trip_duration_list = sorted([int(x) for x in column_to_list(data_list, 2) if x.isdigit()])
min_trip = trip_duration_list[0]
max_trip = trip_duration_list[-1]
mean_trip = sum(trip_duration_list)/len(trip_duration_list)
median_trip = 0.
if(len(trip_duration_list) % 2 == 0):
    median_trip = trip_duration_list[(len(trip_duration_list) // 2) - 1] + trip_duration_list[len(trip_duration_list) // 2]
else:
    median_trip = trip_duration_list[((len(trip_duration_list)+1) // 2) -1]
